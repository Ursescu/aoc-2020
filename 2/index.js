let input = `2-6 c: fcpwjqhcgtffzlbj
6-9 x: xxxtwlxxx
5-6 w: wwwwlwwwh
...
let parsedInput = input.split('\n').map(password => {
    let obj =  {
        character: password.split(': ')[0].split(' ')[1],
        password: password.split(': ')[1],
        number: {
            min: parseInt(password.split(': ')[0].split('-')[0]),
            max: parseInt(password.split(': ')[0].split('-')[1].split(' ')[0]),
        },
    }
    let count = obj.password.split(obj.character).length - 1;
    obj.count = count;
    obj.valid = (count >= obj.number.min) && (count <= obj.number.max);

    return obj;
});


console.log(parsedInput.filter(obj => obj.valid).length);
...
parsedInput = input.split('\n').map(password => {
    let obj =  {
        ...
        password: password.split(': ')[1],
        number: {
            pos1: parseInt(password.split(': ')[0].split('-')[0]) - 1,
            pos2: parseInt(password.split(': ')[0].split('-')[1].split(' ')[0]) - 1,
        },
    };
    let valid = false;
    if (obj.number.pos1 <= obj.password.length) {
        valid |= (obj.password[obj.number.pos1] == obj.character);
    }

    if (obj.number.pos2 <= obj.password.length) {

        if (obj.password[obj.number.pos2] == obj.character) {
            if (valid)
                obj.valid = false;
            else
                obj.valid = true;
